...
class SpellCheck:
    ...
    def known_edits3(self, word, wDict):
        return set(e3 for e2 in self.edits2(word) for e3 in self.edits2(e2) if e3 in wDict)

    #Returns a set of words with the possible edits
    def known_edits4(self, word, wDict):
        return set(e4 for e3 in self.edits3(word) for e4 in self.edits3(e3) if e4 in wDict)
    ...
    def correct(self, word, wDict):
        candidates = self.known([word], wDict) or self.known(self.edits1(word), wDict) or self.known_edits2(word, wDict) or self.known_edits3(word, wDict) or self.known_edits4(word, wDict) or [word] # gets a set of words with the shortest edit distance from the typed word.
        print candidates
