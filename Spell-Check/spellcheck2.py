...
class SpellCheck:
    ...
    def edits3(self, word):
        return set(e3 for e2 in self.edits2(word) for e3 in self.edits2(e2) )

    #Returns a set of words with the possible edits
    def edits4(self, word):
        return set(e4 for e3 in self.edits3(word) for e4 in self.edits3(e3) )
    ...
    class ProbabilityAssigner:
        ...
        def __init__(self, wDict, word, obj):
            self.wDict = wDict
            self.word = word
            self.obj = obj
        ...
        def returnProbability(self, candidate):
            ...
            if candidate[0] == self.word[0]:
                ...
                sameFirstLetter = 0
            if candidate in obj.edits4(word):
                editDistPriority = 0
            if candidate in obj.edits3(word):
                editDistPriority = 1
            if candidate in obj.edits2(word):
                editDistPriority = 2
            if candidate in obj.edits1(word):
                editDistPriority = 3
            if candidate in obj.known([word], wDict):
                editDistPriority = 4
            return (sameFirstLetter, editDistPriority, self.wDict.get(candidate))

    # Highest Level Method
    # Returns the possible word
    def correct(self, word, wDict):
        candidates = self.known([word], wDict) or self.known(self.edits1(word),wDict) or self.known(self.edits2(word),wDict) or self.known(self.edits3(word),wDict) or self.known(self.edits4(word),wDict) or [word] # gets a set of words with the shortest edit distance from the typed word.
        print candidates
        pa = self.ProbabilityAssigner(wDict, word, self)
